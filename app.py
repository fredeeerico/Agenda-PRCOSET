...
if cursor.fetchone()[0] == 0:
    ...
    eventos_teste = [
        ...
        (False, "Evento AGORA", hoje, time(9,0), time(19,30),"Centro Eventos","Centro","Foto","Fred, Ana","Kit","Obs",False,"","","ATIVO"),
    ...
    with st.form("form_evento"):
        ...
        if st.form_submit_button("💾 Salvar"):
            ...
            if evento:
                ...
                observacoes=%s, precisa_motorista=%s, motorista_nome=%s, motorista_telefone=%s,
                status=%s WHERE id=%s
...
with aba_eventos:
    ...
    with col_filtro:
        ...
        filtro_responsavel = st.text_input("👥 Filtrar por responsável", value="")
    ...
    for ev in eventos:
        ...
        if isinstance(data_ev,str):
            data_dt = datetime.strptime(data_ev,"%Y-%m-%d").date()
        else:
            data_dt = data_ev

        if isinstance(hi,str):
            hi_dt = datetime.strptime(hi,"%H:%M:%S").time()
        else:
            hi_dt = hi

        if isinstance(hf,str):
            hf_dt = datetime.strptime(hf,"%H:%M:%S").time()
        else:
            hf_dt = hf
        ...
        elif data_dt==hoje and agora>fim_dt:
            cor_fundo="#d9d9d9"; cor_fonte="#6b6b6b"; opacidade="0.6"; borda="3px solid #FFD700"; badge="<span style='background:#FFD700;color:#000;padding:2px 6px;border-radius:5px;font-weight:bold;'>HOJE</span>"
        elif data_dt==hoje and inicio_dt<=agora<=fim_dt:
